...
def Main(inputTest):
    match test:
        ...
        case "BaseNewtonBall":
            ...
            Earth = Planet(iname = 'Earth', ivel = np.array([0, 1]), ipos = np.array([10, 2]),  iradius = 0.5)
            Mars  = Planet(iname = 'Mars',  ivel = np.array([0, 0]), ipos = np.array([10, 6]), iradius = 0.5)
            state = StateManager(iobjList=[Earth, Mars])

        case "CompNewtonBall":
            gvars.init(ig = 0, idt = 0.0001, iBounceCoef=0.5, iyBound=[-10,10])
            Earth = Planet(iname = 'Earth', ivel = np.array([1, 1]), ipos = np.array([5,  0]), iradius = 0.5)
            Mars  = Planet(iname = 'Mars',  ivel = np.array([0, 0]), ipos = np.array([10, 6]), iradius = 0.5)
            state = StateManager(iobjList=[Earth, Mars])
